...
function audioChip(id){const a=AUDIO[id];return `<div class="audio-chip"><span class="dot"></span><strong>ARCHIVE AUTHENTIQUE</strong><span>${id} · ${esc(a?.[1]||"")}</span></div>`;}
...
function render(){stopAudio();updateHUD();const fn=scenes[state.scene]||scenes.intro;fn();}

const scenes={
  intro(){state.clock="23:56";save();updateHUD();el.screen.innerHTML=`<div class="scene"><div class="scene-kicker">JEUDI · 1999</div><h1>23:56</h1><p class="big">Tu les écoutes depuis des mois.</p><p>Ce soir, tu vas appeler.</p><div class="phone" aria-hidden="true"></div>${btn("☎ APPELER LE STANDARD",()=>setScene("busy1","intro"))}<p class="source-note">Prototype sans image IA. Les voix de Gérard utilisées dans le jeu proviennent des inserts authentiques fournis avec le projet.</p></div>`;},
  busy1(){el.screen.innerHTML=`<div class="scene"><div class="scene-kicker amber">APPEL 01</div><h2>Occupé.</h2><p class="muted">La ligne est déjà prise.</p>${btn("RAPPELER",()=>setScene("busy2"))}</div>`;},
  busy2(){el.screen.innerHTML=`<div class="scene"><div class="scene-kicker amber">APPEL 02</div><h2>Toujours occupé.</h2><p class="muted">Tu raccroches. Tu recomposes.</p>${btn("RAPPELER",()=>setScene("manu_intro"))}</div>`;},
  manu_intro(){el.screen.innerHTML=`<div class="scene"><div class="scene-kicker">LE STANDARD DÉCROCHE</div>${dialogue("MANU — STANDARD","Fun Radio, bonsoir.<br><br>C’est pour quoi ?","manu")}<div class="choices">${choice("A","Passe-moi Gérard. Je veux lui dire que c’est un con.")}${choice("B","Je voudrais participer au débat sur l’euro.")}${choice("C","Je voulais demander si un euro français vaudra autant qu’un euro étranger.")}${choice("D","J’ai une vanne de malade.")}</div></div>`;
    bindChoices({A:()=>{apply({standard:-5});gerbe({reason:"Tu n’as même pas atteint l’antenne.",resume:"manu_intro"});},B:()=>{apply({standard:2});setScene("audition");},C:()=>{apply({standard:10,fun:4});toast("Petit rire au standard.");setScene("pseudo","pseudo");},D:()=>{apply({standard:-4});setScene("audition");}});
  },
  audition(){el.screen.innerHTML=`<div class="scene"><div class="scene-kicker">AUDITION DU STANDARD</div>${dialogue("MANU","Pourquoi je te passe ?","manu")}<div class="choices">${choice("A","Parce que Gérard est nul.")}${choice("B","J’ai vraiment un avis sur l’euro.")}${choice("C","Je voulais savoir si les billets de Monopoly allaient aussi passer à l’euro.")}${choice("D","Passe-moi et tu verras.")}</div></div>`;
    bindChoices({A:()=>{apply({standard:-7});gerbe({reason:"Le standard cherchait quelqu’un capable de jouer, pas seulement d’insulter.",resume:"manu_intro"});},B:()=>{apply({standard:3,fun:-1});setScene("pseudo","pseudo");},C:()=>{apply({standard:10,fun:7});toast("Manu a ri.");setScene("pseudo","pseudo");},D:()=>{apply({standard:-3});if(state.standard<25)gerbe({reason:"Tu as joué au mystérieux un peu trop tôt.",resume:"manu_intro"});else setScene("pseudo","pseudo");}});
  },
  pseudo(){
  ...
  wait(){el.screen.innerHTML=`<div class="scene"><div class="scene-kicker amber">EN ATTENTE</div><h2>${esc(state.pseudo)}</h2>${dialogue("MANU","Bouge pas.","manu")}<p class="muted">Souffle de ligne. Boutons de console. Des voix très loin derrière.</p><div class="choices">${choice("A","Oui.")}${choice("B","Évidemment.")}${choice("C","J’allais raccrocher.")}${choice("D","Je t’écoute.")}</div></div>`;
    bindChoices({A:()=>setScene("onair_intro","onair_intro"),B:()=>{apply({standard:-1});setScene("onair_intro","onair_intro");},C:()=>{apply({standard:-3});setScene("onair_intro","onair_intro");},D:()=>{apply({standard:2});setScene("onair_intro","onair_intro");}});
  },
  ...
  micro_archive(){el.screen.innerHTML=`<div class="scene"><div class="scene-kicker red">GÉRARD CONTINUE…</div><h2>Il veut remettre les choses au clair.</h2><p>Le prototype n’embarque pas encore le débat complet de 1999 : on utilise ici un <strong>insert authentique isolé</strong>, sans prétendre qu’il s’agit d’un extrait continu du débat.</p>${audioChip("GG17")}${btn("ÉCOUTER",async()=>{await playSound("GG17");setScene("understood","understood");})}</div>`;},
  ...
  offair(){state.clock="00:54";save();updateHUD();el.screen.innerHTML=`<div class="scene"><div class="scene-kicker amber">⚫ OFF AIR</div><h2>Pause disque.</h2><p>La voix de Gérard disparaît. D’un coup, tu n’entends plus que la ligne et le standard.</p>${dialogue("MANU","Pour une première, ça va. Mais le flingue pas en dix minutes.","manu")}<div class="choices">${choice("A","J’ai compris.")}${choice("B","Moi je veux surtout le faire gueuler.")}${choice("C","Qu’est-ce qui marche le mieux avec lui ?")}${choice("D","Tony, il fait comment ?")}</div></div>`;
    bindChoices({A:()=>{apply({standard:4});setScene("return_air","return_air");},B:()=>{apply({standard:-8});setScene("return_air","return_air");},C:()=>{apply({standard:6});state.tipStandard=true;save();toast("CONSEIL DU STANDARD · NE CASSE PAS LE JEU");setScene("return_air","return_air");},D:()=>{apply({reputation:4});state.tonyFile=true;save();toast("DOSSIER TONY · VERROUILLÉ POUR PLUS TARD");setScene("return_air","return_air");}});
  },
